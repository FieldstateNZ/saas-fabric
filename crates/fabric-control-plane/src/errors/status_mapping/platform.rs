...
pub(super) fn status(error: &PlatformError) -> StatusCode {
    match error {
        ...
        // 501, and deliberately not 422. The request is well-formed and the
        // version may well be fine; what is missing is a guarantee this
        // platform can make about that kind of artifact at all. An operator
        // retrying with a different version would be answering the wrong
        // question.
        PlatformError::RollbackUnsupported { .. } => StatusCode::NOT_IMPLEMENTED,
