...
use crate::git_integration::service::order::Generation;
...
/// transition that lost it would be one nothing could refuse. The generation is
/// private and `store_and_bind` takes the whole value, so the only generation a
/// transition can be built on is the one read here, first.
