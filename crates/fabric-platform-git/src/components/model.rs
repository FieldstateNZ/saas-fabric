...
impl Artifact {
    ...
    /// if it carries build metadata, the same rule
    /// [`Version::parse`](fabric_platform_management::Version::parse)
    ...
    /// is read with
    /// [`Version::parse_chart`](fabric_platform_management::Version::parse_chart),
    /// which keeps it.
    ///
    ...
    #[must_use]
    pub fn parse_version(&self, text: &str) -> Option<fabric_platform_management::Version> {
        match self {
            Self::Oci { .. } => fabric_platform_management::Version::parse(text),
            Self::Helm { .. } => fabric_platform_management::Version::parse_chart(text),
        }
    }
}
