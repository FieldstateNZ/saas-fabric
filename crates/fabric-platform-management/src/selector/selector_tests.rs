...
use crate::{Channel, Discovery, ReleaseUnit, ResolvedImage, UpdatePolicy, Version};
...
fn a_manual_component_never_writes_however_available_the_update_is() {
    assert_eq!(
        decide(UpdatePolicy::Manual, Channel::Preview, false, &something_newer()),
...
fn a_locked_component_never_writes() {
    assert_eq!(
        decide(UpdatePolicy::Locked, Channel::Preview, false, &something_newer()),
...
fn an_automatic_component_advances_to_what_discovery_found() {
    ...
    let Decision::Advance(chosen) = decide(UpdatePolicy::Automatic, Channel::Preview, false, &found) else {
...
fn only_one_of_the_five_branches_writes() {
    ...
    for (policy, held) in cases {
        assert!(
            matches!(
                decide(policy, Channel::Preview, held, &something_newer()),
                Decision::Stay(_)
            ),
...
fn a_held_component_still_reports_what_it_is_holding_back_from() {
    ...
    assert_eq!(
        decide(UpdatePolicy::Automatic, Channel::Preview, true, &found),
        Decision::Stay(Reason::Held)
    );
    assert_eq!(
