...
/// is not itself a mapping, if the requested chart's key appears under
/// `entries` more than once, if the requested chart's own entries do not
/// match their expected shape, if one of its versions cannot be parsed, or
/// if two of its versions have equal `SemVer` precedence (see [`Version`]'s
