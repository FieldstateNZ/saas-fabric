...
/// not itself a mapping or appears more than once, if the requested chart's
/// key appears under `entries` more than once, or if the requested chart's
/// own entries do not match `Entry`'s shape. A malformed entry under any *other* chart's name
...
impl<'de> Visitor<'de> for TopLevel<'_> {
        ...
        A: MapAccess<'de>,
    {
        ...
        while let Some(key) = map.next_key::<String>()? {
            if key == "entries" {
                if releases.is_some() {
                    // The same refusal `entries` makes one level down: two
                    // statements of what the repository publishes, and no
                    // reason to prefer the later one. Helm's own loader
                    // refuses such an index too.
                    return Err(DeError::custom("entries is listed more than once"));
                }
