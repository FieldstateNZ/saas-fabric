...
#[tokio::test]
async fn an_index_listing_entries_twice_is_refused_rather_than_read_last_wins() {
    // Two statements of what the repository publishes. Taking the later one
    // would answer confidently from half a document; Helm's own loader
    // refuses such an index, and so does this.
    let (base, _recorded) = serving(
        "apiVersion: v1
entries:
  keycloakx:
...
",
    ...
    let failure = charts()
        .versions(&base, "keycloakx")
        .await
        .expect_err("an index that says entries twice is not one to choose within");
...
",
    ...
    // A minute, not a second: the bound is here to catch the regression,
    // which took 43 seconds *optimised* and far longer as `cargo test` builds
    // it, and a wall-clock assertion tight enough to flake on a busy runner
    // would be a test nobody trusts. A healthy read is under two seconds
    // unoptimised, so there is no margin to lose.
    assert!(
