...
fn a_stale_data_source_revision_is_ignored_by_a_full_sync() {
    ...
    registry.apply_all(vec![data_source("sql-au-east-03", 8)]);

    let report = registry.apply_all(vec![data_source("sql-au-east-03", 7)]);
...
fn a_stale_tenant_binding_revision_is_ignored_by_a_full_sync() {
    ...
    registry.apply_all(vec![tenant_binding("acme", 8, "sql-au-east-03")]);

    let report = registry.apply_all(vec![tenant_binding("acme", 7, "sql-au-east-03")]);
