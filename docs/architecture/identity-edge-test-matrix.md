...
| A13 | **The audience mapper is missing, so no genuine token carries the required `aud`** | Edge (it refuses everything) | **Split.** That the mapper is **written** is an **existing test** — `a_declared_client_is_written_with_the_platform_audience_mapper` (`crates/fabric-keycloak/tests/keycloak_adapter_identity.rs:114-126`), asserting the POST body contains `"oidc-audience-mapper"` and `"included.custom.audience":"<configured>"`. That Keycloak **returns** it on `GET /clients`, and that `PUT /clients/{id}` **updates** it rather than requiring `/clients/{id}/protocol-mappers/models`, is **verified in slice 4 against a real Keycloak** (`docs/verification.md`, "Keycloak 26.0.8 probe, 2026-09-06 (issue #61)", findings 2–3) — the echoing fake could confirm either answer and settled neither. A **second** mapper added out of band is the related, separately dangerous case: `audience_mapper` reads `None` rather than picking whichever one Keycloak happens to list first, so it stays visible as drift instead of silently resolving to one of two mappers (`two_audience_mappers_are_observed_as_no_single_audience`, same file). The *ordering* is a **platform obligation** (§G5a) | If the mapper is absent, the edge refuses **every** genuine token with `401`, and ADR 0010 records that it "presents as a signature problem rather than a missing mapper" (`docs/decisions/0010-…:70-74`) |
...
| C4 | **The Keycloak write body omits the PKCE attribute** | The wire test | **Existing test** — `a_declared_client_is_written_with_the_s256_challenge_method` (`crates/fabric-keycloak/tests/keycloak_adapter_identity.rs:102-111`), asserting the POST body contains `"pkce.code.challenge.method":"S256"`. **Mutation-proved** per `docs/delivery.md:68`, recorded in `docs/verification.md`'s mutation table: the insertion was deleted from `provider/declaration.rs`'s `declaration()`, this test failed, and the file was restored | Test failure, not a runtime refusal |
| C5 | **The provider holds a declared client with no PKCE attribute** (created before this slice, or edited by hand) | Reconciliation, as drift | **Existing test** — `a_client_without_a_recognised_challenge_method_is_corrected` (`crates/fabric-reconciliation/src/plan/diff_tests.rs:224-233`). Shared with C6: both read as `challenge_method: None` | Plan contains `UpdateOidcClient`; status becomes `drifted` then `applied` |
| C6 | **The provider holds `plain`, or a value this model otherwise does not recognise** | Reconciliation, as drift | **Existing test** — `a_client_without_a_recognised_challenge_method_is_corrected` (`crates/fabric-reconciliation/src/plan/diff_tests.rs:224-233`), shared with C5. `ObservedOidcClient.challenge_method` is `Option<PkceMethod>`, so an unparseable value reads as `None`, which is not `Some(S256)` — no `Plain` variant is needed anywhere in the model | `UpdateOidcClient` |
| C6a | **The audience mapper was removed by hand** | Reconciliation, as drift | **Existing test** — `a_client_whose_audience_mapper_was_removed_is_corrected` (`crates/fabric-reconciliation/src/plan/diff_tests.rs:259-268`). ADR 0019 §6: without this term the mapper would be written once and could silently disappear, taking the edge's `aud` check down with it | `UpdateOidcClient` |
...
| D13 | **An unparseable URI added to the provider by hand** — `http://evil.example.com/steal` | Reconciliation, as drift | **Existing test, both halves.** The read half: `an_unmodellable_redirect_uri_is_counted_rather_than_dropped` (`crates/fabric-keycloak/tests/keycloak_adapter_identity.rs:141-177`) — `observe::clients` no longer drops an unparseable URI with `.filter_map(… .ok())`; `partition_uris` (`crates/fabric-keycloak/src/provider/observe/clients.rs:81-95`) counts it instead, onto `ObservedOidcClient.unmodellable_redirect_uris`. The reconciliation half: `a_redirect_uri_this_model_cannot_parse_is_drift` (`crates/fabric-reconciliation/src/plan/diff_tests.rs:207-216`) — `matches` (`crates/fabric-reconciliation/src/plan/diff.rs:113-122`) already carries the `unmodellable_redirect_uris == 0` term. **Mutation-proved** for both halves, recorded in `docs/verification.md`'s mutation table: the adapter's old silent drop and the diff's conjunct were each restored in turn, the corresponding test went red, and the file was reverted | Plan contains `UpdateOidcClient`; the declared set is rewritten and the extra URI removed. The **count** is reported, never the value — it is attacker-influenced. The count lives on `ObservedOidcClient` (`crates/fabric-reconciliation/src/provider/observed.rs:78-86`), so the plan names the client to rewrite |
| D13a | **A parseable URI changed by hand** — `https://evil.example.com/callback` | Reconciliation, as drift | **Existing test** — `an_application_client_with_a_changed_redirect_uri_is_updated` (`crates/fabric-reconciliation/src/plan/diff_tests.rs:153-174`). Distinguished from D13 (a URI the model cannot parse at all) and D13b (an extra, undeclared URI the provider also holds): this one already worked because the URI parses | `UpdateOidcClient` |
| D13b | **An extra redirect URI the provider holds** — a legitimate, parseable URI beyond the declared set | Reconciliation, as drift | **Existing test** — `an_extra_redirect_uri_the_provider_holds_is_drift` (`crates/fabric-reconciliation/src/plan/diff_tests.rs:182-193`). `matches` compares the declared and observed redirect-URI sets for equality — this term has held since `4d508e7`, the commit that founded `fabric-reconciliation`, not since this slice — so a legitimate extra entry is drift too: a redirect allow-list is a security boundary, and "the declared set is a subset of what the provider accepts" is not the guarantee this model makes | `UpdateOidcClient` |
...
| D16 | **`post.logout.redirect.uris` is not written** | The wire test | **Existing test** — `a_declared_client_is_written_with_the_registered_uris_as_its_post_logout_set` (`crates/fabric-keycloak/tests/keycloak_adapter_identity.rs:129-138`), asserting the POST body's `attributes` contains `"post.logout.redirect.uris":"+"` — Keycloak's documented value meaning "the registered redirect URIs". One list, so a second cannot drift out of step with it | Test failure, not a runtime refusal |
| D16a | **Keycloak returns `attributes` on read** | Adapter | **Verified in slice 4 against a real Keycloak** (`docs/verification.md`, "Keycloak 26.0.8 probe, 2026-09-06 (issue #61)", findings 2 and 4). `ClientRepresentation` (`crates/fabric-keycloak/src/wire/oidc_client.rs:26-70`) reads `attributes` and `protocolMappers` beside the four fields it always read (slice 4), plus `enabled` and `standardFlowEnabled` (slice 5, `#[serde(default)]`, ADR 0019's written-but-unobserved gap); all round-trip unchanged (`a_declared_native_client_round_trips_through_the_wire_unchanged`, same test file). Without the read-back, D16 and C4 would be write-only assertions and every sweep would rewrite every client forever | Recorded behaviour, in `docs/verification.md` |
...
| E2 | **A declared client the provider holds as confidential** | Reconciliation, as drift | **Existing test** — `a_declared_client_switched_to_confidential_is_corrected` (`crates/fabric-reconciliation/src/plan/diff_tests.rs:290-299`) | `UpdateOidcClient` |
| E3 | **A confidential client declared in a document** | Not representable | **Structural**, guarded by an **existing test**. `OidcClient` has no secret field and no `publicClient: false` path (`crates/fabric-client-model/src/identity/oidc_client.rs:20-27`); `declaration()` hard-codes `public_client: true` (`crates/fabric-keycloak/src/provider/declaration.rs:70`, moved out of `mutate.rs` in slice 4). `a_declared_client_is_written_as_a_public_authorisation_code_client` (**exists**, `crates/fabric-keycloak/tests/keycloak_adapter.rs:162-184`) asserts the body contains no `secret` (`:180-183`) | Deserialisation failure on any secret-shaped key (`deny_unknown_fields`, `oidc_client.rs:29`) |
...
| E8 | **A realm returning more clients than one page holds** | Adapter | **Existing test** — `a_full_page_of_clients_is_refused` (`crates/fabric-keycloak/tests/keycloak_adapter_identity.rs:317-341`), mirroring `roles`' existing refusal (`crates/fabric-keycloak/src/provider/observe.rs:14-20`, `:48-52`). `paths::clients_page` (`crates/fabric-keycloak/src/admin/paths.rs:77-79`) gives `observe::clients` the same bound `paths::roles_page` (`:54-59`) gives `observe::roles`; the unbounded `paths::clients` (`:66-68`) is used only by the create call, which addresses the collection once and never pages it | `ProviderError::Rejected`, naming the cap. Never a partial reconciliation |
...
| E15 | **`v1` clients still get S256 on the next sweep** | Nothing — the deliberate runtime break | **Existing test** — `a_v1_client_is_still_reconciled_with_the_s256_challenge_method` (`crates/fabric-reconciliation/src/plan/diff_tests.rs:243-252`). **This is the break ADR 0019 names**, and the test is what makes it visible rather than surprising | **Not a refusal.** `UpdateOidcClient` on the first sweep after deployment. Every public client not already performing PKCE stops working |
...
(`crates/fabric-keycloak/tests/keycloak_adapter_identity.rs:221-275`): write the
...
observed redirect URIs, PKCE method and audience mapper equal what was
declared, with zero unmodellable URIs, and the client still enabled, still
running the standard flow, and still holding every registered URI as its
post-logout set.

**The `id` splice.** `NewClientRepresentation` carries no `id`
(`crates/fabric-keycloak/src/wire/oidc_client.rs:80-116`) while
`ClientRepresentation` requires one (`:27-31`) — Keycloak generates it. The
test splices `"id":"uuid-native-1"` into the recorded POST body before serving
it as the `GET /clients` response
(`crates/fabric-keycloak/tests/keycloak_adapter_identity.rs:234`), and nothing
else — any other edit and the test stops proving that the write and the read
agree.
