...
| D13 | **An unparseable URI added to Keycloak by hand** — `http://evil.example.com/steal` | Reconciliation, as drift | **Split.** `a_redirect_uri_this_model_cannot_parse_is_drift` in `crates/fabric-reconciliation/src/plan/diff_tests.rs` remains a **proposed test**: `matches` (`crates/fabric-reconciliation/src/plan/diff.rs:76-80`) has not yet gained the term. The read half is now an **existing test** — `an_unmodellable_redirect_uri_is_counted_rather_than_dropped` (`crates/fabric-keycloak/tests/keycloak_adapter_identity.rs:141-177`): `observe::clients` no longer drops an unparseable URI with `.filter_map(… .ok())`; `partition_uris` (`crates/fabric-keycloak/src/provider/observe/clients.rs:76-90`) counts it instead, onto `ObservedOidcClient.unmodellable_redirect_uris`. **Mutation-proved** for that half, recorded in `docs/verification.md`'s mutation table: the old silent drop was restored, the test failed, and the file was reverted | Plan contains `UpdateOidcClient`; the declared set is rewritten and the extra URI removed. The **count** is reported, never the value — it is attacker-influenced. The count lives on `ObservedOidcClient` (`crates/fabric-reconciliation/src/provider/observed.rs:36-47`), so the plan names the client to rewrite |
...
| D16a | **Keycloak returns `attributes` on read** | Adapter | **Verified in slice 4 against a real Keycloak** (`docs/verification.md`, "Keycloak 26.0.8 probe, 2026-09-06 (issue #61)", findings 2 and 4). `ClientRepresentation` (`crates/fabric-keycloak/src/wire/oidc_client.rs:24-57`) now reads `attributes` and `protocolMappers` as well as the four fields it always read; both round-trip unchanged (`a_declared_native_client_round_trips_through_the_wire_unchanged`, same test file). Without the read-back, D16 and C4 would be write-only assertions and every sweep would rewrite every client forever | Recorded behaviour, in `docs/verification.md` |
...
declared, with zero unmodellable URIs.

**The `id` splice.** `NewClientRepresentation` carries no `id`
(`crates/fabric-keycloak/src/wire/oidc_client.rs:59-103`) while
