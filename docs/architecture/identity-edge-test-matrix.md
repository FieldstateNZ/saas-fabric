...
Test names are proposals in this repository's voice. Names in `code` marked
**(exists)** are already in the tree at `bc1f58c`, with the line they are on.
...
and so is every bracketed authority that is not the loopback `[::1]`.

### D0 — normalisation

| # | Case | Refused by | Provability | Expected |
|---|---|---|---|---|
| D0a | **`https://LOCALHOST:5173/cb` under `claimedHttps`** | Model validation, after lower-casing | **Proposed test** — `an_upper_case_loopback_host_is_still_a_loopback_host`. Today the host is not examined at all under `https` (`crates/fabric-client-model/src/identity/redirect_uri/authority.rs:63-65`), so this parses and would classify as `Https` without the lower-casing | `InvalidField`, `400` on write / `500` on read. Named as `Loopback` under a strategy that admits only `Https` |
| D0b | **`https://ADMIN.CORP.INTERNAL/cb` under `claimedHttps`** | Model validation, after lower-casing | **Proposed test** — `an_upper_case_internal_host_is_still_a_private_network_host`. Today `.internal` is matched case-sensitively (`authority.rs:105`), so this classifies as an ordinary public host | `InvalidField`, `400` / `500` |
| D0c | **`http://LOCALHOST:5173/cb` under `development`** | Nothing — permitted after lower-casing | **Proposed test** — `an_upper_case_loopback_host_is_accepted_over_plain_http`. **Refused today**: `LOOPBACK.contains(&host)` is case-sensitive (`authority.rs:104`), so this is a `BadBoundary` at `bc1f58c` | **Not a refusal**, after this slice. A widening, and deliberate |
| D0d | **`HTTPS://www.example.com/cb`** | Nothing — permitted after lower-casing | **Proposed test** — `an_upper_case_scheme_is_the_same_scheme`. **Refused today**: `strip_prefix("https://")` is case-sensitive (`authority.rs:63`) | **Not a refusal**, after this slice |
...
| # | Case | Refused by | Provability | Expected |
|---|---|---|---|---|
| D1 | **Loopback URI under `claimedHttps`** — `http://localhost:5173/callback` | Model validation | **Proposed test** — `a_development_callback_is_refused_under_the_production_strategy` | `DesiredStateError::InvalidField`, `spec.identity.clients`, naming the strategy, the URI's kind and what the strategy admits. `400` on write, `500` on read |
| D1a | **`https://` loopback under `claimedHttps`** — `https://localhost:5173/callback` | Model validation | **Proposed test** — `a_loopback_host_is_not_a_claimed_https_callback_even_over_tls`. **The host rule's sharpest edge**: the scheme is right and the classification is still `Loopback` | `InvalidField`, `400` |
| D1b | **`https://` `.internal` under `claimedHttps`** — `https://admin.corp.internal/cb` | Model validation | **Proposed test** — `a_private_network_host_is_not_a_claimed_https_callback_even_over_tls` | `InvalidField`, `400` |
| D1c | **`https://localhost/cb` under `development`** | Nothing — permitted | **Proposed test** — `a_loopback_callback_may_be_served_over_tls`. A developer running a local TLS proxy writes exactly this; it is `Loopback`, so `Development` is the strategy that holds it | **Not a refusal.** Named because "the scheme is https, therefore the strategy is claimedHttps" is the intuition the partition breaks |
| D2 | **`https://` public URI under `development`** | Model validation | **Proposed test** — `a_public_callback_is_refused_under_the_development_strategy`. Refused rather than waved through as "stricter than needed": the strategy is a statement about what the client *is* | `InvalidField`, `400` |
| D3 | **`.internal` URI under `claimedHttps`** (plain HTTP) | Model validation | **Proposed test** — `a_private_network_callback_is_refused_under_the_production_strategy` | `InvalidField`, `400` |
...
| D5 | **A private-use-scheme URI inside another strategy** — `nz.fieldstate.slipway:/cb` under `development` | Classification, then validation | **Proposed test** — `a_private_use_scheme_is_not_a_loopback_redirect` | `InvalidField`, `400` |
| D5a | **`nz.fieldstate.slipway://localhost/cb`** | Classification — it is `PrivateUseScheme`, not `Loopback` | **Proposed test** — `a_private_use_scheme_with_a_loopback_authority_is_still_a_private_use_scheme`. **This is the row scheme-first exists for.** A host-first partition would classify it by the `localhost` in its authority and hand a native application's callback the entitlement a development HTTP callback has. The authority of a private-use URI is not a network location; RFC 8252 §7.1's own examples put nothing there | Under `development`: `InvalidField`, `400` — the kind is not admitted. Under `customScheme`: refused as deferred (D4). Never `Loopback` |
| D6 | **Empty `uris` list** | Model validation | **Proposed test** — `a_strategy_with_no_callback_could_never_sign_a_user_in`, replacing today's `an_application_client_with_no_redirect_uri_is_refused` (**exists**, `crates/fabric-client-model/src/identity/validation_tests.rs:70`) over `validation.rs:78-90` | `InvalidField`, `400` |
| D7 | **Wildcard in the host** — `https://*.example.com/callback` | `RedirectUri::try_new` | **Existing test** — `refuses_a_wildcard_in_the_host` (`crates/fabric-client-model/src/identity/redirect_uri.rs:160`). **Mutation-proved**, and now against **two** guards: `characters::check` refuses a `*` that is not the final character (`crates/fabric-client-model/src/identity/redirect_uri/characters.rs:60`), and the registered-domain rule will not take `*` as a label character. The test carries `http://*.lucentroot.internal/callback` as well, which reaches only the first — remove either guard and it still has to fail | `IdentifierError::DisallowedCharacter` → `Malformed`, `400` |
| D7a | **Trailing wildcard under `claimedHttps`** — `https://www.example.com/*` | Strategy validation | **Proposed test** — `a_wildcard_callback_is_refused_under_the_production_strategy`. The URI parses today (`redirect_uri.rs:128`) — the strategy is what refuses it, naming RFC 9700 §2.1's exact-matching requirement and that Universal/App Links need exact URLs | `InvalidField`, `400` |
| D7b | **Trailing wildcard under `privateNetwork`** | Strategy validation | **Proposed test** — `a_wildcard_callback_is_refused_under_the_private_network_strategy` | `InvalidField`, `400` |
| D7c | **Trailing wildcard under `development`** | Nothing — permitted | **Proposed test** — `a_trailing_path_wildcard_is_the_one_place_a_development_callback_may_use_one` | **Not a refusal.** The one permitted path wildcard |
...
| D9 | **Userinfo in the authority** — `http://x.internal@evil.example.com/` | `authority::reject_userinfo` (`crates/fabric-client-model/src/identity/redirect_uri/authority.rs:101`) | **Existing test** — `a_public_host_cannot_smuggle_the_private_domain_through_userinfo` (`authority_tests.rs:61`), covering both schemes | **`Unadmitted`**, `400`. Not `BadBoundary`: the value has no bad character in it, and "must start and end with an alphanumeric character" sends its author hunting for a typo that is not there. The message says a redirect URI has no legitimate use for credentials |
| D10 | **Plain HTTP on a public host** — `http://www.example.com/callback` | `host_kind::classify`, the `!secure` arm (`crates/fabric-client-model/src/identity/redirect_uri/host_kind.rs:105`) | **Existing tests** — `refuses_plain_http_anywhere_else` (`redirect_uri.rs:155`), `plain_http_is_refused_on_a_public_host` (`authority_tests.rs:45`), plus `plain_http_on_a_public_host_names_the_boundary_rather_than_a_typo` (`authority_tests.rs:221`), which asserts the message | **`Unadmitted`**, `400`, naming the boundary: `https for a public host; plain http only on loopback or a .internal host`. Was `BadBoundary`, which is the same misdirection as D9 |
| D11 | **`.internal` as a path, not a host** — `http://evil.example.com/.internal` | `authority::check` (`authority.rs:44-49`, `:79-81`) | **Existing test** — `a_public_host_cannot_smuggle_the_private_domain_through_the_path` (`authority_tests.rs:45`) | `BadBoundary`, `400` |
...
| D14 | **Loopback with no port** — `http://127.0.0.1/callback` under `development` | Nothing — permitted, meaning any port | **Proposed test** — `a_loopback_callback_with_no_port_admits_any_port` (RFC 8252 §7.3) | **Not a refusal** |
| D14a | **Loopback with `*` in the port** — `http://127.0.0.1:*/callback` | `RedirectUri::try_new` (`crates/fabric-client-model/src/identity/redirect_uri/characters.rs:50`) | **Existing test** — `a_wildcard_port_is_refused_because_a_portless_loopback_callback_already_matches_any_port` (`crates/fabric-client-model/src/identity/redirect_uri.rs:182`), over both spellings: `:*/callback` and the bare `:*`, which puts its `*` last and would otherwise pass as a trailing path wildcard. An earlier draft of this matrix had this row as **not a refusal**; the probe below is what changed it | **`Unadmitted`** → `Malformed`, `400`. The message names what to write instead: over `http` a portless loopback callback already matches any port, and over `https` the port has to be named |
| D14b | **What any-port actually means, over each scheme** | Keycloak | **Observed on Keycloak 26.0.8, 2026-09-06**; re-verified on LucentRoot's 26.7.2 by the platform lane (§G17). Over `http`, a loopback URI registered **without a port** matches the same path on any port — Keycloak compares no port for it, which is RFC 8252 §7.3's requirement met. Over `https`, and whenever a port is written under either scheme, the match is **exact**. `http://127.0.0.1:*/cb` matches nothing at all, which is why D14a is a refusal. The in-repo half is `a_loopback_callback_with_no_port_admits_any_port` (`crates/fabric-client-model/src/identity/client_rules_tests.rs:227`), which proves only that the portless spelling is admitted — the matching is Keycloak's | **Not a refusal.** The asymmetry is recorded rather than modelled: nothing in this repository can enforce what Keycloak compares, so the ADR states it and this row names where it came from |
...
| D15 | **IPv6 loopback** — `http://[::1]:5173/callback` under `development` | Nothing — permitted | **Proposed test** — `the_ipv6_loopback_address_is_a_development_callback`. Not accepted today: `LOOPBACK` is `["localhost", "127.0.0.1"]` (`authority.rs:12`) and an IPv6 literal is refused over plain HTTP by design (`authority.rs:85-88`). Requires **bracket-aware host parsing** — `host` splits on the first colon (`authority.rs:90`), turning `[::1]:5173` into `[`. The comment at `:85-88` reasons that the ambiguity does not matter *because* an IPv6 literal is never loopback-by-name; that stops being true here and the comment is rewritten with the rule | **Not a refusal**, after this slice |
| D15a | **`127.0.0.2`** — loopback to the OS, not to this model | Classification | **Proposed test** — `only_three_loopback_hosts_are_a_development_callback`, over all three near-misses | Refused, and the message **names the boundary** — "loopback is `127.0.0.1`, `::1` or `localhost`" — rather than reading as a parse failure. `400` / `500` |
| D15b | **`[::ffff:127.0.0.1]`** — the IPv4-mapped spelling | Classification | Same **proposed test**. **Accepted today under `https://`**, because that arm checks only userinfo (`authority.rs:63-65`), so this is a deliberate narrowing: a claimed-HTTPS entitlement satisfied by an address that never leaves the machine is the entitlement failing to mean anything | Refused, naming the boundary. `400` / `500` |
| D15c | **`localhost.localdomain`** — resolves to loopback on many machines | Classification | Same **proposed test**. An entitlement that can only be recognised by resolving a name is not a declaration | Refused, naming the boundary. `400` / `500` |
...
### D17–D19 — the registered-domain rule, and the two spellings only the author can resolve
...
| # | Case | Refused by | Provability | Expected |
|---|---|---|---|---|
| D17 | **Any IP literal under `https`** — `93.184.216.34`, `134744072`, `0x08080808`, `[2001:db8::1]` | `registered_domain::check` | **Existing test** — `an_ip_literal_is_never_a_claimed_https_callback` (`crates/fabric-client-model/src/identity/redirect_uri/authority_tests.rs:173`), which now carries every spelling in this block | `Unadmitted` → `Malformed`, `400`. The message names "a registered domain". A Universal Link and an App Link are claimed against a domain; an address is not one |
...
| D17b | **`https://0x.0x.0x.0x/cb`** | `registered_domain::check`, at the "ends in a number" rule | Same test, plus `a_name_ending_in_a_number_is_an_address_a_browser_would_dial` (`registered_domain.rs:208`). The final label begins `0x`, which is the URL Standard's test for a host that is an IPv4 candidate rather than a name | Refused, `400` |
| D17c | **`https://１２７．０．０．１/cb`** — fullwidth digits and full stops | `registered_domain::check`, at the ASCII rule | Same test, plus `an_internationalised_host_is_refused_in_favour_of_its_a_label` (`registered_domain.rs:163`). UTS-46 maps this back to `127.0.0.1` | Refused, `400`, and the message asks for the **A-label** (`xn--`) form — that is the name the browser resolves and the name the claim is made against |
| D17d | **`https://[::1%25lo0]/cb`** — a bracketed zone id | `authority::reject_brackets` (`crates/fabric-client-model/src/identity/redirect_uri/authority.rs:132`) | Same test. A zone id names an interface only one machine has, so `::1%25lo0` is not `::1` and the callback is not a declaration any other machine could act on | Refused, `400`, naming what a bracketed authority is |
...
| E13 | **A private-use scheme in a `v1` list** | The migrator | **Proposed test** — `a_v1_client_with_a_private_use_scheme_must_be_migrated_by_hand`. Cannot arise from a document `v1` could hold — `authority::check` refuses those schemes today (`.../redirect_uri/authority.rs:60-76`) — so the test constructs it directly. The arm exists so the migrator stays **total** once `RedirectUri` widens in this same slice | `DesiredStateError::Migration`, `400` / `500` as E12 |
