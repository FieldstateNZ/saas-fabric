...
(loopback → `.internal` → `Https`, and `Https` only if the scheme is `https`).
Scheme and host are **lower-cased before every test**.
...
| D7 | **Wildcard in the host** — `https://*.example.com/callback` | `RedirectUri::try_new` | **Existing test** — `refuses_a_wildcard_in_the_host` (`crates/fabric-client-model/src/identity/redirect_uri.rs:142`). **Must be mutation-proved against the new parser**: the wildcard-port widening (D14a) touches the same rule at `redirect_uri.rs:82-98`, so this is the regression that widening most plausibly causes. Remove the guard, watch it fail, restore | `IdentifierError::DisallowedCharacter` → `Malformed`, `400` |
...
| D8 | **`javascript:` scheme** | `authority::check` | **Existing tests** — `refuses_a_javascript_scheme` (`redirect_uri.rs:150`) and `a_scheme_that_is_not_http_is_refused` (`redirect_uri/authority_tests.rs:74`), which **already covers `data:` and `ftp://` as well** (`:75-78`); only `file:` is new. Must still fail *after* the private-use-scheme rule is added — this is the regression that rule most plausibly causes. **Mutation-proved** | `IdentifierError::BadBoundary`, `400` |
| D9 | **Userinfo in the authority** — `http://x.internal@evil.example.com/` | `authority::check` (`crates/fabric-client-model/src/identity/redirect_uri/authority.rs:93-100`) | **Existing test** — `a_public_host_cannot_smuggle_the_private_domain_through_userinfo` (`authority_tests.rs:53`), covering both schemes | `BadBoundary`, `400` |
| D10 | **Plain HTTP on a public host** — `http://www.example.com/callback` | `authority::check` | **Existing tests** — `refuses_plain_http_anywhere_else` (`redirect_uri.rs:137`), `plain_http_is_refused_on_a_public_host` (`authority_tests.rs:37`) | `BadBoundary`, `400` |
...
| D14a | **Loopback with `*` in the port** — `http://127.0.0.1:*/callback` under `development` | Nothing — permitted, meaning any port | **Proposed test** — `a_wildcard_port_is_how_a_loopback_callback_says_any_port`. **Not representable today**: `check_characters` permits `*` only in the final position (`crates/fabric-client-model/src/identity/redirect_uri.rs:82-98`), so this requires the parser to widen. The parser widens **universally**; the strategy narrows (D14b) | **Not a refusal** |
| D14b | **A wildcard port under any other strategy** | Model validation | **Proposed test** — `a_wildcard_port_is_only_meaningful_on_loopback`. This is the narrowing that pays for D14a's widening: a wildcard port on a public host matches every port on that host, which is the over-broad entry `RedirectUri` exists to refuse (`crates/fabric-client-model/src/identity/redirect_uri.rs:13-20`) | `InvalidField`, `400`, naming `development` as the only strategy that admits one |
