...
| Runtime binding publication | The producer exists now: `fabric-runtime-publication` (ADR 0018) owns the wire contract for the three published documents and a filesystem adapter proven against the real runtime and Data API. What does not exist is a caller — no Kubernetes adapter writing the ConfigMaps a deployment mounts, no scheduled process deciding a revision and calling `publish()`, and no provisioner input (`ProvisionedPlacement`) to fill a published `DataSource` or tenant binding from. `ResourceSource` / `JsonFileSource` remain the runtime's unchanged consumer side. |
